      ...
      db.ref("/customers")
        ...
        .once("value", (snapshot) => {
          if (snapshot.val() == null && snapshot.val() == undefined) {
            ...
            twiml.message(
              `Hello user. I don't know you yet. Please go to the website to get registered!. Thanks`
            );
            ...
            db.ref("/customer_messages")
              .orderByChild("customer")
              .equalTo(Object.keys(customer)[0])
              .limitToLast(1)
              .once("value", (customerMessageSnapshot) => {
                // No previous sent message
                if (
                  customerMessageSnapshot.val() == null ||
                  customerMessageSnapshot.val() == undefined
                ) {
                  ...
                  db.ref("/messages")
                    ...
                    .once("value", (messageSnapshot) => {
                      if (
                        messageSnapshot.val() != null &&
                        messageSnapshot.val() != undefined
                      ) {
                        ...
                        if (msg[Object.keys(msg)[0]].stringFunc) {
                          if (msg[Object.keys(msg)[0]].stringValue == "name") {
                            showMessage = showMessage.replace(
                              /_/g,
                              customer[Object.keys(customer)[0]].fullName
                            );
                          } else {
                            db.ref("/coupons")
                              .orderByKey()
                              .limitToLast(3)
                              .once("value", (couponSnap) => {
                                if (couponSnap.val() != null) {
                                  ...
                                  var interval = setInterval(function () {
                                    ...
                                    sendMessageService
                                      .SendMessage(
                                        ...
                                        `${index + 1}.` +
                                          coupons[index].key.title,
                                        coupons[index].key.artWorkLink
                                      )
                                      .then(
                                        (res) => {},
                                        (error) => {
                                  ...
                                  else twiml.message(`${showMessage}`);
                                  res.set("Content-Type", "text/xml");
                                  return res.status(200).send(twiml.toString());
                                }
                              });
                  ...
                  let answers =
                    customerMassage[Object.keys(customerMassage)[0]].message
                      .answers;
                  // Expects an answer for last massage
                  console.log(
                    "Checking prevous message expect a answer or not"
                  );
                  if (
                    customerMassage[Object.keys(customerMassage)[0]].message
                      .expect
                    ...
                    if (answers != undefined) {
                      ...
                      if (answers.length != 0) {
                        ...
                        answers.forEach((element) => {
                          ...
                          console.log(
                            "checking customer answers are matching to expecting answers"
                          );

                          if (
                            element.answer.toLowerCase().trim() ==
                            ...
                            db.ref("/messages")
                              .orderByChild("id")
                              .equalTo(element.replyMessage)
                              .once("value", (replySnap) => {
                                reply = replySnap.val();

                                let expectedAnswers = "";

                                if (
                                  reply[Object.keys(reply)[0]].answers !=
                                  undefined
                                ) {
                                  expectedAnswers =
                                    "\n Please provide one of following answers.\n";
                                ...
                                db.ref("/customer_messages").push(
                                  customerNewMessage
                                );

                                let showMessage =
                                  reply[Object.keys(reply)[0]].message;
                                ...
                                switch (element.func) {
                                  case "AddOrder/SendMessage":
                                    console.log(
                                      "Answer type was AddOrder/SendMessage"
                                    );
                                    ...
                                    console.log(
                                      customer[Object.keys(customer)[0]].userId,
                                      "obj"
                                    );

                                    console.log(
                                      "Checking customer previous orders"
                                    );

                                    db.ref("/whatsapp_order")
                                      ...
                                      .once("value", (orderSnap) => {
                                        if (orderSnap.val() == null) {
                                          ...
                                          db.ref("/coupons")
                                            ...
                                            .once("value", (couponSnap) => {
                                              if (couponSnap.val() != null) {
                                                ...
                                                console.log(
                                                  "Adding new order with 1 quantity"
                                                );
                                                switch (answer) {
                                                  case "1":
                                                    if (
                                          ...
                                          console.log(
                                            "Check customer previous order finished or not"
                                          );
                                          ...
                                          if (
                                            ...
                                            console.log(
                                              "Customer previous order not finished"
                                            );
                                            ...
                                            db.ref("/coupons")
                                              ...
                                              .once("value", (couponSnap) => {
                                                ...
                                                console.log(
                                                  "Adding the new coupon to customer order"
                                                );
                                                if (couponSnap.val() != null) {
                                                  switch (answer) {
                                                    case "1":
                                            ...
                                            console.log(
                                              "Customer previous order has finished"
                                            );
                                            let newOrder = Object.assign({
                                              customer:
                                                customer[
                                  ...
                                  case "UpdateOrder/SendMessage":
                                    ...
                                    db.ref("/whatsapp_order")
                                      ...
                                      .once("value", (orderSnap) => {
                                        console.log(
                                          "Updateing the customer order with quantity"
                                        );
                                        console.log(
                                          orderSnap.val(),
                                          "QUantity update db values"
                                        );
                                        if (orderSnap.val() != null) {
                                          let lastOrder = orderSnap.val();
                                ...
                                if (reply[Object.keys(reply)[0]].stringFunc) {
                                  if (
                                    reply[Object.keys(reply)[0]].stringValue ==
                                    "name"
                                  ) {
                                    showMessage = showMessage.replace(
                                      /_/g,
                        ...
                        if (found == false) {
                          console.log(
                            "customer messages are not matching for expecting answers"
                          );
                          twiml.message(
                            `Sorry I can't understand your answer. Please repeat`
                          );
                          res.set("Content-Type", "text/xml");
                          return res.status(200).send(twiml.toString());
                        }
                    ...
                    db.ref("/messages")
                      ...
                      .once("value", (messageSnapshot) => {
                        if (
                          ...
                          if (msg[Object.keys(msg)[0]].stringFunc) {
                            if (
                              msg[Object.keys(msg)[0]].stringValue == "name"
                            ) {
                              showMessage = showMessage.replace(
                                /_/g,
                                customer[Object.keys(customer)[0]].fullName
